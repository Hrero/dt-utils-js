...
function getTwoArrayObiectKeys(arr, brr, option) { // 数组对象1和数组对象2，从1中赋值id相同的元素 keys: Array到b数组中,返回brr
    const {
        aid,
        bid,
        keys
...
function findMaxDuplicate(str) { // const str = 'this is a fe test at toutia o on September'; findMaxDuplicate(str);
    str = str.replace(/\s+/g, "")
    console.log(str);
    let maxCar = '', maxValue = 1;
    let mapStr = new Map();
